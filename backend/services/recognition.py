...
def process_incoming_face_image(image_bytes, db_conn=None, model_name=None,
    ...
    try:
        ...
        response = {
            ...
            "faces": faces,
            # Every bbox above is in the pixel space of the image that was
            # uploaded, and the live client downscales frames before posting
            # them. Without these dimensions a caller cannot map a box back onto
            # its own video element, so the overlay lands beside the face.
            "image_size": {"width": int(probe_image.shape[1]),
                           "height": int(probe_image.shape[0])},
            "summary": summary,
